...
for kappa_ind = 1:kappa_no
    for noise_ind = 1:noise_no
        ...
        for ind = 1:ir_it_max
            ...
            % solve
            if precond
                xn1 = lsqr(A,single(f),lsqrtol,lsqrit,R);
                xn2 = lsqr(A,single(r),lsqrtol,lsqrit,R);
                
                RTinvg = (R')\single(g);
                rn1 = lsqr(ATprec,RTinvg,lsqrtol,lsqrit);
            else
                xn1 = lsqr(A,single(f),lsqrtol,lsqrit);
                xn2 = lsqr(A,single(r),lsqrtol,lsqrit);

                rn1 = lsqr(A',single(g),lsqrtol,lsqrit);
            end
